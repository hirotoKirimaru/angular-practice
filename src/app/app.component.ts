...
export class AppComponent implements OnInit {
  title = 'front';
  public emailControl: FormControl = new FormControl("", [Validators.required, Validators.email, this.domainValidator]);
  ...
  ngOnInit() {
    ...
    this.nameOrEmailControl.valueChanges.pipe(
  ...
  domainValidator(control: FormControl): { [key: string]: any } | null {
    ...
    if (email && email.indexOf('@') !== -1) {
      ...
      if (domain !== 'example.com') {  // 特定のドメインをチェック
